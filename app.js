...
app.use('/games/search', function(req, res) {
  ...
  request({ url: "https://www.boardgameatlas.com/api/search?client_id=1Joh8La5B1&name=" + req.query.gamename + "&limit=10&skip=" + (page-1)*10} , function(err, response, jsonString) {
...
app.use('/games/browse/:pageid', function(req, res) {
  var page = parseInt(req.params.pageid, 10)
  request({ url: "https://www.boardgameatlas.com/api/search?client_id=1Joh8La5B1&limit=28&skip=" + (page-1)*28} , function(err, response, jsonString) {
    var json = JSON.parse(jsonString)
    var gameNameList = json.games.map(function(e) {
        var discountPerc = parseFloat(Math.round(100*(1.0 - e.price/e.msrp))).toFixed(0);
        var over = discountPerc < 0;
...
app.use('/games/:gameid', function(req, res) {
  ...
  request({ url: "https://www.boardgameatlas.com/api/search?client_id=1Joh8La5B1&ids=" + req.params.gameid} , function(err, response, jsonString) {
    var json = JSON.parse(jsonString)
